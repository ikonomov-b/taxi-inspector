...
2a. **Open, and the one place the mode-S change bills more rather than less.** The deadband is the larger of the movement floor and either endpoint's accuracy, while the plausibility rule allows each endpoint its full accuracy, so at 20 m reported accuracy a stationary vehicle whose consecutive fixes differ by more than 20 m — which the inspection's own noise model calls routine at that accuracy — has each difference billed as distance, and the baseline advances so the next one bills too. Twenty seconds of standing still at 20 m accuracy bills 570 m, about 2 per minute on the test tariff, where the superseded hysteresis billed nothing. `RideEngineTest.jitter wider than the deadband is billed as distance` pins the figure. The fix is the veto already named in the plan's section 4.2 residuals — reject a close whose chord is under the sum of the endpoint accuracies, or whose new baseline returns within the deadband of the old one — and it should be decided before commit 2 rather than waiting for field data, since a unit test already demonstrates it.
