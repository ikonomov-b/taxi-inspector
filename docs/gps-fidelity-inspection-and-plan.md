...
>
> **Further amended 2026-09-12** (separate from the commit-4 rewrite above): the waiting crossover this section describes as tariff-derived (`perMin × 1000 / (perKm × 60)`) is no longer read by `reconcile`, `labelOnHold`, or `labelOnTimeClose` at all. A real field drive showed the tariff-derived crossover billing ordinary slow city traffic entirely as waiting time, and OIML R21 confirms the crossover is meant to vary by jurisdiction rather than follow from the two money rates. `Tariff` now carries a fourth, user-edited, stored component, `waitingCrossoverKilometersPerHour` (product default 8 km/h — not a regulatory value; Bulgaria specifies 5, other EU states allow up to 20), and every `crossover` symbol below should be read as that stored value. The old derived getter is deleted. A second aggregate, `ActiveRide.travelledDistanceMeters` / `RideSummary.travelledDistanceMeters`, now also commits alongside `distanceMeters` on every closed interval (both `Distance`- and `Time`-won), so a saved ride can show both what was billed and what was actually driven; it is a lower bound on the physical path (a chord sum with a deadband), not a route measurement. See `project-memory.md`'s durable fare/GPS decisions for the full record, and Room schema version 3 / `MIGRATION_2_3`.
...
- `Tariff.crossoverSpeedMetersPerSecond`: `+∞` when `perKm == 0`, else `perMin × 1000 / (perKm × 60)`; `0.0` when `perMin == 0`. Label and trace only.
  **Superseded 2026-09-12**: this getter is deleted. `Tariff` instead stores `waitingCrossoverKilometersPerHour` directly (product default `"8"`, `Tariff.DEFAULT_WAITING_CROSSOVER_KILOMETERS_PER_HOUR`), and `waitingCrossoverMetersPerSecond` is now a plain unit conversion of that stored value, not a derivation from the other two rates. See the amendment at the top of this document.
