...
- A single-tariff variant of the product, with no company name at all, is retained deliberately. `ui/tariff/` and `RoomRideRepository.saveTariff` are kept building and tested but unwired, so an anonymous edition can be assembled without rebuilding the editor; its one tariff is stored as an unnamed company row, so both variants share the version-2 schema. Do not delete it as dead code.
