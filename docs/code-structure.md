...
- A `TaxiCompany` is editable local metadata around exactly one `Tariff`; its user-entered name is not a verified business identity and never enters fare arithmetic. Its stored duplicate-rejection key is the trimmed, locale-independently case-folded name (`TaxiCompany.nameKey`), so the rule is one deterministic value rather than a database collation.
...
Room stores up to ten `TaxiCompanyEntity` rows, one `AppSettingsEntity` containing the selected company id, one `ActiveRideEntity`, and `RideSummaryEntity` rows. `RoomRideRepository` is the sole data access point. Company creation enforces the ten-row limit transactionally and rejects overflow without automatic eviction. Names are stored trimmed, limited to 80 characters, and checked for case-insensitive duplicates through a unique index on the stored `nameKey` column rather than a `COLLATE NOCASE` index, because NOCASE folds ASCII only and would admit two indistinguishable non-ASCII labels. Company writes therefore abort on a name-key conflict; they must never use a replacing insert, which would delete the colliding company instead of rejecting the write. `selectedCompanyId` deliberately carries no foreign key: a selection is cleared by the transaction that deletes its company, and Start rejects a stale selection with an actionable message, so the database must not silently null it instead. Company creation, selection, editing, and confirmed deletion are rejected while an active ride exists. Deleting the selected company clears the selection; Start remains unavailable until another existing company is explicitly selected.
