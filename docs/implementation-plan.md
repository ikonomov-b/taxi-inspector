...
**The reference meter is not exact ground truth.** OIML R21:2007 §3.2.1.2/3.2.2 sets the maximum permissible error for an installed, in-service taximeter at ±0.2 % on elapsed time and ±2 % on distance — a fully compliant, freshly-verified meter can legally sit anywhere in that band, and a real one may be older or drift further before its next inspection. Treat a real taxi ride's reported fare/distance/time as bounded by roughly that tolerance rather than as the single correct answer: a disagreement inside it may be the reference meter's own legal slack, not an app error, and confirming an app defect from one ride's signed error alone is not sound. Collect several rides before concluding the app is biased one way, and record the reference meter's own verification/calibration status if it is known.

### Step 8.3: Resolve taximeter matching and bounded gap reconstruction

- Identify whether the intended reference meter uses single application below/above a cross-over speed, simultaneous distance/time application, or a jurisdiction-specific rule. Do not assume the current 0.8/1.3 m/s stationary hysteresis matches its tariff switch. **Amended 2026-09-12:** that hysteresis no longer exists — the engine applies the time tariff below a cross-over speed and the distance tariff above it — and the cross-over is now a per-company value the user sets (`Tariff.waitingCrossoverKilometersPerHour`, default 8 km/h), not one derived from the two money rates. So matching a specific meter is partly a configuration question: every reference-meter comparison must record the crossover the company was configured with, and a mismatch may be a setting rather than an engine defect.
...
- Repeat the reference-taximeter drives after implementation. Accept the change only if it materially reduces absolute error without an unacceptable positive-charge bias — measured against the ±0.2 %/±2 % OIML tolerance band above, not against the reference meter's raw reading as if it were exact.
